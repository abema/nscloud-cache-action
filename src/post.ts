    ...
    if (!useSymlinks) {
        ...
        for (const p of cachePaths) {
            ...
            if (!st.isSymbolicLink()) {
                core.warning(`${p.mountTarget}: was linked to the cache volume, but is not a symlink anymore. Did another action (e.g. checkout) overwrite it?`);
                foundProblems = true;
                continue;
            }
        ...
        if (foundProblems) {
            core.info(`See https://namespace.so/docs/actions/nscloud-cache-action for more info.`);
        }
    }
}